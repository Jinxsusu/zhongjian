...
export default {
  ...
  data() {
    return {
      ...
      treeOptons: {
        dataSource: {
          serviceInstance: tapp.services.base_Navigation.getTreeAll,
  ...
  methods: {
    renderTreeNodeContent(h, {
      ...
      return (<span class="custom-tree-node">
			<span>{data.self.title}({data.name})  </span>
			<span>
    ...
    handleNodeClick(dataItem, node, el) {
      this.selectedItem = dataItem;
		...
		handleDrop(draggingNode, dropNode, dropType, ev) {
			let self = this;
			 tapp.services.base_Navigation.updateParentIdAndSortIndex({
				 ...
				 self.$notify.success({
					 title: '菜单导航变更父节点或顺序成功，注销后重新登陆系统后修改生效',
					 message: '系统提示'
				 });
    ...
    doBatchDelete() {
      ...
      self.$confirm('此操作将永久删除' + ids.length + '个菜单导航, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        tapp.services.base_Navigation.batchDelete(ids).then(function(result) {
          self.$notify.success({
            title: '系统删除成功',
    ...
    doExportExcel() {
      this.$refs.searchReulstList.exportCSV('菜单导航列表');
    },
    doRefresh() {
