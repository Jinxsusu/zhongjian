      ...
      <el-card class="box-card">
        <div slot="header" class="clearfix" v-if="disabled==false">
          <div style="float: left; padding: 3px 0">
            <div  style="float:left; width:88px;height:36px;line-height:16px;">
              <div   :id="`filePicker_${_uid}`">选择文件</div>
            </div>
        ...
        <uploader ref="uploader" :url="fileUploadUrl" :uploadButton="`#filePicker_${_uid}`" :multiple="false" @fileChange="fileChange" @uploadStart="onUploadStart" @uploadBeforeSend="onUploadBeforeSend" @progress="onProgress" @success="onSuccess" @error="onError" @uploadError="onUploadError"></uploader>
...
export default {
  ...
  methods: {
    ...
    getFileDownloadUrl() {
      ...
      return window.SITE_CONFIG['serverUrl'] + "/pmtapi/base_AssetManagement/download?id=" + this.file.id;
    },
    handlePreview() {
      if (!this.file) {
        return;
