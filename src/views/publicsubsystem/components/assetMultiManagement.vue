    ...
    <el-col :span="showCategoryTree?15:24" v-if="showAttachmentGrid">
      <el-card class="box-card">
        ...
        <div v-if="viewMode=='card'">
          <item-card-view :fileList="itemGridOptions.dataSource" @click="handlePreview" @select="handleItemSelectionChange" :disabled="disabled"></item-card-view>
        </div>
      </el-card>
      <br/>
      ...
      return ( <
        ...
        loadViewFileList() {
          if (!this.selectedAssetCategoryId || !this.businessDocId) {
            this.itemGridOptions.dataSource.length == 0;
            return;
          }
          let requestModel = {
            'businessDocId': this.businessDocId,
